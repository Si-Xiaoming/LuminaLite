...
#include <vector>
#include <cstdint>
// offer uint32_t uint8_t etc. type
#include <cassert>

uint32_t pack_color(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a = 255) {
    // pack the color into a single uint32_t
    // a is the alpha channel, which is packed into the highest byte
    // r is the red channel, which is packed into the lowest byte
    return (a << 24) + (b << 16) + (g << 8) + r;
}

void unpack_color(const uint32_t& color, uint8_t& r, uint8_t& g, uint8_t& b, uint8_t& a) {
    // unpack the color from a single uint32_t
    r = (color >> 0) & 255;
    g = (color >> 8) & 255;
    b = (color >> 16) & 255;
    a = (color >> 24) & 255;
...
int main() {
    ...
    const float fov = 3.141592653 / 3.; // field of view
    ...
    drop_ppm_image("./static_fov_raycast.ppm", framebuffer, win_w, win_h);
    system("start static_fov_raycast.ppm");
